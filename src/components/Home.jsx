import React, { useState, useEffect } from 'react';
import './Home.css';
import { useNavigate } from 'react-router-dom';

const Home = () => {
  const [message, setMessage] = useState('Learn React.js');
  const navigate = useNavigate();

  // Time-stop function to alternate the message every 2 seconds
  useEffect(() => {
    const messages = ['Learn React.js', 'Learn Java Full Stack'];
    let index = 0;

    const timer = setInterval(() => {
      index = (index + 1) % messages.length;
      setMessage(messages[index]);
    }, 2000); // Change the message every 2 seconds

    return () => clearInterval(timer); // Cleanup timer
  }, []);

  // Navigate to the Courses page
  const handleExploreCourses = () => {
    navigate('/course'); // Assuming the route to the courses page is '/courses'
  };

  return (
    <div className="home">
      <h1>Welcome to SR-coder Coding Learning Platform</h1>
      <h2 className="message">{message}</h2>

      <button className="explore-btn" onClick={handleExploreCourses}>
        Explore Courses
      </button>

      <div className="block-container">
        {/* YouTube Channel Block */}
        <div className="block">
          <img
            src="data:image/png;base64,"
            alt="Join YouTube Channel"
            className="block-image"
          />
          <div className="block-content">
            <h3>Join our YouTube Channel</h3>
            <p>🎥 Join SR-coder on YouTube! 🎥

Full Stack tutorials (React, Java, Node)
Real-world project demos
Coding tips & challenges</p>
            <a href="https://www.youtube.com/@SR-coder25" target="_blank" rel="noopener noreferrer">
              <button className="join-btn">Subscribe Now</button>
            </a>
          </div>
        </div>

        {/* WhatsApp Community Block */}
        <div className="block">
          <img
            src="data:image/jpeg;base64,"
            alt="Join WhatsApp Community"
            className="block-image"
          />
          <div className="block-content">
            <h3>Join our WhatsApp Community</h3>
            <p>💬 Join SR-coder WhatsApp Community! 💬

Connect with fellow coders
Get real-time support
Share tips & projects</p>
            <a href="https://chat.whatsapp.com/H7ddIDphEJA0Ed3qk0aIsYq" target="_blank" rel="noopener noreferrer">
              <button className="join-btn">Join Now</button>
            </a>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Home;
